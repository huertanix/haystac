...
(function(){
	...
	//var termSourceUrls = ['http://www.google.com/trends/hottrends?date=2010-06-03', 'http://www.google.com/trends/hottrends?date=2010-06-02'];
	...
	function setTerms(termSauce)
	{
		...
		//if (termSourceUrls[currentSourceIndex])
	...
	function webSearch(resultsPage)
	{
					...
					(function(searchTerm, isUserTerm)
					{
						...
						else
						{
							GM_xmlhttpRequest({
								...
								var tossedClickThruUrls = new Array(randomNumberBetwixt(0, clickThroughUrls.length));
								...
								// Uncommenting this next line (the alert) creates a buffer of time for a clickthrough to complete...
								//alert('tossed urls: ' +  tossedClickThruUrls.length);
	
								for (var y=0; y<tossedClickThruUrls.length; y++)
								{	
									// extract real URL... again
									var realUrl = tossedClickThruUrls[y].match(/url=[^\&]*\&/g)[0]; // this will failwhale is no match is found...	
		...
		// TODO: generate new terms and call webSearch again after a random interval of time...
	}
	...
	btnSearch.addEventListener('click', function(){ webSearch(null); }, false);
	...
	function checkHayProjVersion()
	{
		...
		if (window.currentHayProjVersion)
		{
			var arrLatestVersion = window.currentHayProjVersion.split('.');
			// The internet does not know how to access @version, it seems...
			var arrThisVersion = [0,1,0]; // This will have to updated on *every* release... </emo>
			
			if (arrLatestVersion[0] >= arrThisVersion[0])
			{
				if (arrLatestVersion[1] >= arrThisVersion[1])
				{
					if (arrLatestVersion[2] > arrThisVersion[2])
					{
		...
		{
			// FAIL
			alert('Version number is over 9000.');
		}
	}
