...
function webSearch(withUserTerm)
{
	...
	ghtTerms1 = ghtTerms(randomDate(ghtMinDate, new Date()), randomNumberBetwixt(5, 10));
	...
	ghtTerms2 = ghtTerms(randomDate(ghtMinDate, new Date()), randomNumberBetwixt(5, 10));
...
function ghtTerms(trendDate, termCount)
{
	// Set term limit
	...
	GM_xmlhttpRequest({
    		method: 'GET',
    		url: 'http://www.google.com/trends/hottrends?date=' + trendDate.getFullYear().toString() + '-' + (trendDate.getMonth()+1).toString() + '-' + trendDate.getDate().toString(),
    		headers: {
       	 		'User-agent': navigator.userAgent
    		},
    		onload: function(responseDetails) 
    		{
    			return parseGhtPage(responseDetails.toString(), termCount);
    		}
		});
		
	//alert('blah: ' + arrGhtTerms[0]); //DEBUG
	////return arrGhtTerms;
}
